...
labels = ['Easy', 'Hard']
values = [easy_rate, hard_rate]
x = np.arange(len(labels))

with plt.xkcd():
    fig, ax = plt.subplots(figsize=(6, 4), dpi=100)
    bars = ax.bar(x, values, color=['#AEC6CF', '#FFB347'], width=0.5, edgecolor='white')
    ax.set_xticks(x)
    ax.set_xticklabels(labels, fontsize=12)
    ax.set_ylabel('Success Rate', fontsize=12)
    ax.set_ylim(0, 1)
    ax.set_title('Attack Success Rate by Difficulty', fontsize=14)
